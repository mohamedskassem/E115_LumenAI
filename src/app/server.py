...
def cleanup_cache_dirs(delete_db=False):
    ...
    if delete_db and os.path.exists(DB_PATH):
        ...
        try:
            ...
            logging.warning(
                f"Could not determine/add schema cache file for deletion: {e}"
            )
        ...
        if os.path.exists(dir_path):
            try:
                ...
                if e.errno == errno.EBUSY:
                    ...
                    err_msg = (
                        f"Could not clean up directory {dir_path} due to OS error: {e}"
                    )
...
def load_global_data(force_regenerate=False):
    ...
    logging.info(
        f"Attempting to load global data from DB: {DB_PATH} (Force regenerate: {force_regenerate})"
    )
...
cleanup_cache_dirs(
    delete_db=False
)  # Clean caches on startup, keep existing DB if present
if os.path.exists(DB_PATH):
    logging.info(
        "Existing database found on startup. Attempting to load data globally."
    )
...
def handle_query():
    ...
    try:
        ...
        if not agent.is_initialized:
            logging.error(
                f"Query received for uninitialized agent (Chat ID: {chat_id})"
            )
            return (
                jsonify({"error": f"Agent for chat {chat_id} is not ready."}),
                500,
            )  # Internal error state

        logging.info(
            f"Received query for chat '{chat_id}' using model '{model_name}': "
            f"'{user_question}'"  # noqa: E501
        )

        # --- Title Generation (if requested and needed) ---
        logging.debug(
            f"Checking title generation for chat {chat_id}: should_generate={should_generate_title}, current_agent_title='{agent.chat_title}'" # noqa: E501
        )  # DEBUG LOG
        if should_generate_title and agent.chat_title == "New Chat":
            logging.info(
                f"Request to generate title for chat {chat_id} based on: '{user_question}'"
            )
            ...
            if llm_instance_for_title:
                ...
                if new_title:
                    ...
                    logging.info(
                        f"Successfully updated title for chat {chat_id} to: '{new_title}'"
                    )
                else:
                    logging.warning(
                        f"Failed to generate title for chat {chat_id}. Keeping default."
                    )
            else:
                logging.warning(
                    f"Could not get LLM instance to generate title for chat {chat_id}."
                )
        ...
        logging.info(
            f"Sending response for chat {chat_id}: Type={response.get('type')}"
        )
...
def upload_data():
    ...
    if del_errors:
        logging.error(
            f"Errors occurred during pre-upload cleanup: {del_errors}. Aborting upload."
        )
        # Return 500 if cleanup failed critically
        return (
            jsonify(
                {
                    "error": f"Failed to clean up existing data before upload: {del_errors}",
                    "warnings": del_warnings,
                }
            ),
            500,
        )
    ...
    for file in files:
        if file and file.filename.lower().endswith(".csv"):
            ...
            try:
                ...
                return (
                    jsonify({"error": f"Failed to save uploaded file {file.filename}"}),
                    500,
                )
        else:
    ...
    try:
        ...
        for csv_path in saved_files:
            ...
            table_name = "".join(c if c.isalnum() else "_" for c in table_name).strip(
                "_"
            )
            if not table_name:
                table_name = f"data_{uuid.uuid4().hex[:6]}"  # Ensure unique fallback
            ...
            for encoding in encodings_to_try:
                try:
                    ...
                    logging.warning(
                        f"Failed read {csv_path} encoding '{encoding}'. Trying next..."
                    )
                except Exception as e:
            ...
            df.columns = [
                "".join(c if c.isalnum() else "_" for c in str(col)).strip("_")
                or f"col_{i}"
                for i, col in enumerate(df.columns)
            ]
            # Ensure no empty column names
            df.columns = [
                f"col_{i}" if not name else name for i, name in enumerate(df.columns)
            ]
    ...
    if not load_global_data(force_regenerate=True):  # Force regenerate on new upload
        ...
        return (
            jsonify(
                {"error": "Data loaded to DB, but global indexing failed. Check logs."}
            ),
            500,
        )
    ...
    if not initial_chat_id:
        ...
        return (
            jsonify(
                {
                    "error": "Data loaded and indexed, but failed to create initial chat session."
                }
            ),
            500,
        )
...
def remove_data():
    ...
    if errors:
        ...
        return (
            jsonify(
                {
                    "message": f"Attempted to remove data. Critical errors encountered: {errors}",
                    "errors": errors,
                    "warnings": warnings,
                }
            ),
        ...
        success_message = (
            "Successfully removed all data (DB, caches) and chat sessions."
        )
        if warnings:
            success_message += (
                " Note: Some non-critical cleanup warnings occurred (e.g., EBUSY)."
            )
